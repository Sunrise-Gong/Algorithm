function solution(lottos, win_nums) {
    ...
    // 최저 등수 = 7 - 맞힌 개수 (가 7 이라면 7등은 없으므로 6으로 변경(맞힌 개수가 0개인 경우))
    const min = 7 - matchedCount === 7 ? 6 : 7 - matchedCount;
    
    // 최고 등수 = 최저등수 - 0의 개수 (가 0 이라면 1로 변경 (0이 6개인 경우))
    const max = min - zeroCount === 0 ? 1 : min - zeroCount;
...
민우가 구매한 로또 번호 6개가 44, 01, 0, 0, 31, 25   라고 가정해보겠습니다.

당첨 번호               6개가 31, 10, 45, 1, 6, 19 라면,
...
알아볼 수 없는 두 개의 번호를
각각 10, 6이라고 가정하면 3등에 당첨될 수 있습니다.

3등을 만드는 다른 방법들도 존재합니다.
하지만, 2등 이상으로 만드는 것은 불가능합니다.

알아볼 수 없는 두 개의 번호를
각각 11, 7이라고 가정하면 5등에 당첨될 수 있습니다.
...
입출력 예 #3
민우가 구매한 로또의 번호와 당첨 번호가 모두 일치하므로,
최고 순위와 최저 순위는 모두 1등입니다.
